...
package provide TkFunBt 1.0

namespace eval ::tkfbt::funbutton {
    namespace export funbutton
...
proc ::tkfbt::funbutton::funbutton {w img txt callback} {
    ...
    bind $w <Configure> {::tkfbt::funbutton::conf  %W %w %h}
    bind $w <<ThemeChanged>> {::tkfbt::funbutton::themechanged %W}
    bind $w <Enter>     {::tkfbt::funbutton::enter %W}
    bind $w <Leave>     {::tkfbt::funbutton::leave %W}
    bind $w <Button-1>  {::tkfbt::funbutton::click %W}
...
proc ::tkfbt::funbutton::gonormal {w state} {
}

proc ::tkfbt::funbutton::click {w} {
    ...
    if {$state == "normal"} {
        ...
        if {[set ${ns}::${w}_over] == 1} {
            ::tkfbt::funbutton::start_color_anim $w $buttoncolor_selected_over
        } else {
            ::tkfbt::funbutton::start_color_anim $w $buttoncolor_selected
        }
        ...
        # start animation to the new button color
        if {[set ${ns}::${w}_over] == 1} {
            ::tkfbt::funbutton::start_color_anim $w $buttoncolor_normal_over
        } else {
            ::tkfbt::funbutton::start_color_anim $w $buttoncolor_normal
        }
    } else {
        #
...
proc ::tkfbt::funbutton::start_color_anim {w target} {
    variable ns
    ...
    after 0 ::tkfbt::funbutton::animate    \
        $w $target                       \
...
proc ::tkfbt::funbutton::animate {         \
    ...
    #
    if {$newcolor != $target} {
        after $anim_update_after ::tkfbt::funbutton::animate               \
                            $w $target                                   \
...
proc ::tkfbt::funbutton::themechanged {w} {
    ::tkfbt::funbutton::setcolors
}

proc ::tkfbt::funbutton::conf {win w h} {
...
if {$::argv0 eq [info script]} {
    ...
    set img [image create photo -file "icons/16x16/status-audio-volume-low.png" -format png]
    set txt "Hello"
    ::tkfbt::funbutton::funbutton .root.canv $img $txt bcl
    #funbutton .root.canv $img $txt bcl
